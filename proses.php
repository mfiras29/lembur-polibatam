...
elseif (isset($_GET['absen'])) {
	if($_GET['absen']==1){
		...
		$sql = "INSERT INTO data_absen (
			...
			?,
			?)";
		if ($statement = $koneksi->prepare($sql)) {
			$statement->bind_param(
				"siiisss", $_SESSION['user'], $month, $day, $day_tgl, $hour, $status,$jam_klr
				);
		...
		$hour = date("H.i")." WIB";
		$status = "Menunggu";
		$sql = "INSERT INTO data_absen (id_absen , id_form_lembur , absen_jam_keluar) VALUES (NULL,'$idfl','$getTime')";
		$query = mysqli_query($koneksi, $sql)or trigger_error('Query failed: ' . $sql, E_USER_ERROR);
		var_dump($getTime);
		header("Location: absen.php");
...
elseif (isset($_POST['simpan_note'])) {
	...
	if ($note !== "") {
		...
		if ($statement = $koneksi->prepare($sql)) {
			$statement->bind_param(
				"siiiiss", $id_note, $id_user, $month, $day, $day_tgl, $note, $status
				);
			if ($statement->execute()) {
